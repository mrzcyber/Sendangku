...
@section('title', 'Edit Service Package - Sendangku')
@section('meta_description', 'Edit package layanan Sendangku.')
@section('body_class', 'font-sans bg-muted min-h-screen overflow-x-hidden text-foreground')

@section('content')
@php
    $selectedService = $servicePackage->service;
@endphp
...
<div class="flex h-screen max-h-screen flex-1 overflow-hidden">
  <main class="flex-1 lg:ml-[280px] flex flex-col bg-muted min-h-screen overflow-x-hidden">
    <div class="flex-1 overflow-y-auto p-5 md:p-8">
      <form action="{{ route('admin.service-package.update', $servicePackage) }}" method="POST" class="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px] gap-6">
        @csrf
        @method('PUT')
        ...
        <div class="flex flex-col rounded-3xl border border-border bg-white shadow-sm overflow-hidden">
          <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 border-b border-border">
            <div>
              <h3 class="font-bold text-lg text-foreground">Edit Package</h3>
              <p class="text-sm text-secondary mt-1">Perbarui package untuk service {{ $selectedService->name ?? '-' }}.</p>
            </div>
          ...
          <div class="p-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="flex flex-col gap-2">
                ...
                <input type="hidden" name="service_id" value="{{ old('service_id', $servicePackage->service_id) }}">
              ...
              <div class="flex flex-col gap-2">
                ...
                <input
                  ...
                  name="title"
                  type="text"
                  value="{{ old('title', $servicePackage->title) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="flex flex-col gap-2">
                ...
                <input
                  ...
                  type="text"
                  value="{{ old('whatsapp_number', $servicePackage->whatsapp_number) }}"
            ...
            <div class="flex flex-col gap-2">
              ...
              <textarea
                ...
                required
              >{{ old('benefit', $servicePackage->benefit) }}</textarea>
            ...
            <div class="flex flex-col gap-2">
              ...
              <textarea
                ...
                required
              >{{ old('whatsapp_message', $servicePackage->whatsapp_message) }}</textarea>
          ...
          <div class="flex flex-col rounded-3xl border border-border bg-white shadow-sm overflow-hidden">
            ...
            <div class="p-6 space-y-4">
              ...
              <label class="flex cursor-pointer items-center justify-between gap-4 rounded-2xl ring-1 ring-border bg-muted px-4 py-4 text-sm font-bold text-foreground has-[:checked]:ring-primary has-[:checked]:bg-primary/10 has-[:checked]:text-primary transition-all">
                ...
                <input type="checkbox" name="populer" value="1" class="size-5 accent-primary" @checked((string) old('populer', (string) (int) $servicePackage->populer) === '1')>
              </label>
              @error('populer')
                <p class="text-sm font-medium text-error">{{ $message }}</p>
              @enderror
