  ...
  <main class="flex-1 lg:ml-[280px] flex flex-col bg-muted min-h-screen overflow-x-hidden">
    ...
    <div class="flex-1 overflow-y-auto p-5 md:p-8">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 class="text-foreground text-2xl font-bold mb-1">Tambah Ticket</h1>
          <p class="text-secondary text-sm">Buat tipe tiket baru yang akan tampil untuk pengunjung.</p>
        </div>
      ...
      <form action="{{ route('admin.ticket-type.store') }}" method="POST" class="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px] gap-6">
        @csrf
          ...
          <div class="flex flex-col rounded-3xl border border-border bg-white shadow-sm overflow-hidden">
            ...
            <div class="p-6">
              <div class="rounded-2xl bg-muted ring-1 ring-border p-5">
                ...
                <p class="font-bold text-foreground">Ticket baru</p>
                <p class="text-sm text-secondary mt-1">Data ticket akan tersimpan setelah form dikirim.</p>
              </div>
            </div>
          </div>
