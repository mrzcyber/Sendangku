...
@section('title', 'Edit User - Sendangku')
@section('meta_description', 'Edit akun anggota, kasir, atau petugas tiket Sendangku.')
  ...
  <main class="flex-1 lg:ml-[280px] flex flex-col bg-muted min-h-screen overflow-x-hidden">
    ...
    <div class="flex-1 overflow-y-auto p-5 md:p-8">
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h1 class="text-foreground text-2xl font-bold mb-1">Edit User</h1>
          <p class="text-secondary text-sm">Perbarui data akun dan akses user.</p>
        </div>
      ...
      <form action="{{ route('admin.user.update', $user->id) }}" method="POST" class="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px] gap-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="flex flex-col gap-2">
                ...
                >
                  ...
                  <option value="kasir" @selected($selectedRole === 'kasir')>Kasir</option>
                  <option value="tiket" @selected($selectedRole === 'tiket')>Tiket</option>
                </select>
                @error('role')
                  <p class="text-sm font-medium text-error">{{ $message }}</p>
                @enderror
        ...
        <aside class="flex flex-col gap-6">
          <div class="flex flex-col rounded-3xl border border-border bg-white shadow-sm p-6 gap-4">
            <div class="size-12 rounded-2xl bg-primary/10 flex items-center justify-center">
              <i data-lucide="user-pen" class="size-6 text-primary"></i>
            </div>
            <div>
              <h3 class="font-bold text-lg text-foreground">Simpan Perubahan</h3>
              <p class="text-sm text-secondary mt-1">Password hanya berubah jika field password baru diisi.</p>
            </div>
